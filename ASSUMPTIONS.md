...
1. **Read-only list endpoints on Stage 2.** The submitted backend had GET-by-id for POs/SOs/journals and no movement history route. A fresh console reload cannot reconstruct operator lists from those alone without stashing IDs locally (which is not backend state). Additive GET list/snapshot/movement routes wrap existing tables/`getProductStock`. Writes, RBAC, 409/422 codes, recon formula, and locking are unchanged.
...
4. **Second session** holds a second JWT in component state (does not replace the signed-in user). Its Reserve button POSTs `/sales-orders` as that token. Session A then revalidates stock from the API.
